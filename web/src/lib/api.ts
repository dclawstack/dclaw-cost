...
// so no CORS issues and the backend URL is never exposed to the browser.
...
export interface BillingRecord {
  id: string; cloud_account_id: string; resource_id: string; resource_type: string;
  service: string; region: string; cost_usd: number; usage_quantity: number;
  usage_unit: string; billing_period_start: string; billing_period_end: string;
  tags: Record<string, string> | null; created_at: string;
}

export interface Budget {
  id: string; name: string; amount_usd: number;
  period: "daily" | "weekly" | "monthly" | "quarterly" | "annual";
  scope: "account" | "service" | "tag" | "team"; scope_value: string;
  alert_threshold_pct: number; status: "active" | "inactive";
  created_at: string; updated_at: string;
}
...
export interface CostAlert {
  ...
  current_spend_usd: number; projected_spend_usd: number | null;
...
export interface ResourceRecommendation {
  ...
  current_cost_usd: number; estimated_savings_usd: number; confidence: number;
  details: Record<string, unknown> | null; status: "open" | "applied" | "dismissed";
  created_at: string; updated_at: string;
}
...
export interface WasteItem {
  ...
  estimated_monthly_waste_usd: number; last_used_at: string | null;
  status: "open" | "remediated" | "ignored"; created_at: string;
}

export interface TopService { service: string; cost_usd: number; }
export interface BudgetUtilization { budget_id: string; name: string; amount_usd: number; spent_usd: number; used_pct: number; }
export interface DashboardStats {
  total_monthly_spend: number; projected_monthly_spend: number;
  total_savings_opportunities: number; active_alerts_count: number;
  top_services: TopService[]; budget_utilization: BudgetUtilization[];
}

export interface RIPlan {
  id: string; cloud_account_id: string; service: string; resource_type: string;
  commitment_type: string; term_years: number; monthly_on_demand_cost: number;
  monthly_ri_cost: number; upfront_cost: number; monthly_savings: number;
  annual_savings: number; break_even_months: number; discount_pct: number;
  status: string; created_at: string; updated_at: string;
}

export interface TagAllocation { id: string; tag_key: string; tag_value: string; team: string; project: string | null; created_at: string; updated_at: string; }
...
export interface CopilotMessage { id: string; session_id: string; role: "user" | "assistant"; content: string; created_at: string; }
...
export const getCloudAccount = (id: string) => fetchJson<CloudAccount>(`/api/v1/cloud-accounts/${id}`);
...
export const listBillingRecords = (params: { cloud_account_id?: string; service?: string; region?: string; start?: string; end?: string; limit?: number; offset?: number; } = {}) => {
...
export const ingestBillingRecords = (records: Omit<BillingRecord, "id" | "created_at">[]) =>
  fetchJson<{ created: number }>("/api/v1/billing-records/ingest", { method: "POST", body: JSON.stringify({ records }) });

export const listBudgets = (limit = 20, offset = 0) => fetchJson<Budget[]>(`/api/v1/budgets/?limit=${limit}&offset=${offset}`);
...
export const deleteBudget = (id: string) => fetchJson<void>(`/api/v1/budgets/${id}`, { method: "DELETE" });

export const listCostAlerts = (params: { severity?: string; acknowledged?: boolean; budget_id?: string; limit?: number; offset?: number; } = {}) => {
  const q = new URLSearchParams(); Object.entries(params).forEach(([k, v]) => v != null && q.set(k, String(v)));
  return fetchJson<CostAlert[]>(`/api/v1/cost-alerts/?${q}`);
};
export const acknowledgeAlert = (id: string) => fetchJson<CostAlert>(`/api/v1/cost-alerts/${id}/acknowledge`, { method: "PUT" });

export const listRecommendations = (params: { recommendation_type?: string; status?: string; cloud_account_id?: string; limit?: number; offset?: number; } = {}) => {
  const q = new URLSearchParams(); Object.entries(params).forEach(([k, v]) => v != null && q.set(k, String(v)));
...
export const updateRecommendationStatus = (id: string, status: string) =>
  fetchJson<ResourceRecommendation>(`/api/v1/recommendations/${id}/status`, { method: "PUT", body: JSON.stringify({ status }) });

export const listWasteItems = (params: { waste_type?: string; status?: string; cloud_account_id?: string; limit?: number; offset?: number; } = {}) => {
  const q = new URLSearchParams(); Object.entries(params).forEach(([k, v]) => v != null && q.set(k, String(v)));
  return fetchJson<WasteItem[]>(`/api/v1/waste-items/?${q}`);
};
export const updateWasteItemStatus = (id: string, status: string) =>
  fetchJson<WasteItem>(`/api/v1/waste-items/${id}/status`, { method: "PUT", body: JSON.stringify({ status }) });
...
export const listRIPlans = (accountId: string, status?: string) =>
  fetchJson<RIPlan[]>(`/api/v1/ri-plans/${accountId}${status ? `?status=${status}` : ""}`);
...
export const deleteAllocationRule = (id: string) => fetchJson<void>(`/api/v1/cost-allocation/rules/${id}`, { method: "DELETE" });
...
export const deleteSaasSubscription = (id: string) => fetchJson<void>(`/api/v1/saas/${id}`, { method: "DELETE" });
...
export const listSpotStrategies = (accountId: string, status?: string) =>
  fetchJson<SpotStrategy[]>(`/api/v1/spot-strategy/${accountId}${status ? `?status=${status}` : ""}`);
...
// FinOps Reports
export const getFinOpsReport = (months = 12) => fetchJson<FinOpsReport>(`/api/v1/reports/finops?months=${months}`);

// Copilot
export const getCopilotHistory = (sessionId: string) => fetchJson<CopilotMessage[]>(`/api/v1/copilot/history/${sessionId}`);
