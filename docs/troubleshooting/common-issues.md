...
# Check events
kubectl get events -n dclaw-cost --sort-by='.lastTimestamp'

# Verify database connection
kubectl exec -n dclaw-cost deployment/dclaw-cost-backend --   python -c "import asyncio; from sqlalchemy import text; ..."
```
...
2. Check the connection string secret:
   ```bash
   kubectl get secret dclaw-cost-db-credentials -n dclaw-cost
   ```
